try:
    # Patch the buggy hash function in the cluster setup to match the JS implementation

    import base64
    import hashlib
    from cluster_setup import ClusterObject, ClusterSetup
    from __main__ import PluginRemote

    # https://github.com/koush/scrypted/commit/3416347a1f79a0e0b065eeda42fb35fb50893ede
    def computeClusterObjectHash(self: ClusterSetup, o: ClusterObject) -> str:
        m = hashlib.sha256()
        m.update(
            bytes(
    ...
    for frame in stack:
        ...
        if self_var and isinstance(self_var, PluginRemote):
            if hasattr(self_var, "clusterSetup"):
                print("Patching PluginRemote.clusterSetup.computeClusterObjectHash")
                self_var.clusterSetup.computeClusterObjectHash = types.MethodType(computeClusterObjectHash, self_var.clusterSetup)
            break
except:
